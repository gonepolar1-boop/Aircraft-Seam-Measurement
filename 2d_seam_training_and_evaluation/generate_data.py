...
from analyze_seam_mask import extract_seam_geometry
...
class Augmenter:
    ...
    def _rotate(self, img, mask, params=None):
        ...
        img_out = cv2.warpAffine(
            ...
            flags=cv2.INTER_LINEAR,  # 图像用线性插值，平滑
            borderMode=cv2.BORDER_REFLECT  # 边界镜像填充，避免黑边
        )
        mask_out = cv2.warpAffine(
            mask, transform_matrix, (self.img_shape[1], self.img_shape[0]),
            flags=cv2.INTER_NEAREST,
            borderMode=cv2.BORDER_CONSTANT,
            borderValue=0  # 填充值为 0（背景）
        )
    ...
    def _fake_seam(self, img, mask, params=None):
        ...
        img_out -= fake * random.uniform(20, 45)  # 比真实缝浅
...
def generate_base_sample(image_height, image_width):
    ...
    img = base + grad  # 1. 金属背景：高灰度 + 低频渐变
    # =========================
    # 2. 随机定义对缝中心线: 任意方向 + 全局轻微弯曲
    # =========================
    ...
    s = (xx - cx) * np.cos(theta) + (yy - cy) * np.sin(theta)  # 沿缝方向坐标 s
    d = -(xx - cx) * np.sin(theta) + (yy - cy) * np.cos(theta)  # 垂直缝方向坐标 d
    # =========================
    # 3. 全局光滑曲线（不是绝对直线）
    #    局部近似直，全局有轻微弯曲
    # =========================
    ...
    quad = random.uniform(-1e-4, 1e-4) * (s ** 2)  # 再加一个很弱的二次项，模拟大尺度曲面/投影
    center_offset = curve_amp * np.sin(2 * np.pi * s / curve_period + curve_phase) + quad  # 平滑弯曲中心线偏移
    # =========================
    # 4. 缝宽变化 + 两侧不完全平行
    # =========================
    ...
    asym = random.uniform(-0.6, 0.6)  # 左右边缘不完全对称
    # =========================
    # 5. 边缘轻微抖动（高频小扰动）
    # =========================
